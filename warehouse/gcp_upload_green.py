...
BASE_URL = "https://d37ci6vzurychx.cloudfront.net/trip-data/green_tripdata_{}-{}.parquet"
DATASET_ID = "de_dataset" # Replace with your dataset
TABLE_ID = "green_tripdata"
STAGING_TABLE = "green_tripdata_tmp"
...
def run_data_pipeline(request):
    ...
    for m in range(start_month, end_month + 1):
        ...
        try:
            print(f"--- Processing green_tripdata_{year_str}-{month_str}.parquet ---")
            # 1. Download raw parquet (stream directly)
            response = requests.get(url, stream = True)
            response.raise_for_status()
            
            # 2. Upload raw parquet to GCS
            blob = bucket.blob(f"green_tripdata_{year_str}-{month_str}.parquet")
            # blob.upload_from_string(response.raw, content_type='application/octet-stream')
            with blob.open("wb") as f:
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)

            print(f"Streamed to gs://{BUCKET_NAME}/green_tripdata_{year_str}-{month_str}.parquet")
            
            # 3. Load GCS to Staging Table (Overwrite staging each time)
            uri = f"gs://{BUCKET_NAME}/green_tripdata_{year_str}-{month_str}.parquet"
            ...
            USING (
                SELECT 
                    ...
                    'green_tripdata_{year_str}-{month_str}.parquet' AS filename,
                    ...
                    * EXCEPT(passenger_count, RatecodeID, payment_type, trip_type)
